...
async function initOrders() {
  ...
  try {
    const res = await fetch(`${Utils.API_BASE_URL}/orders?userId=${encodeURIComponent(user.email)}`);
    if (res.ok) {
    ...
    console.warn('⚡ API Backend tidak terjangkau untuk orders, menggunakan localStorage...');
  }

  if (orders.length === 0) {
    const allOrders = JSON.parse(localStorage.getItem('tektok_orders') || '[]');
    orders = allOrders.filter(o => o.userId === user.email);
  }
